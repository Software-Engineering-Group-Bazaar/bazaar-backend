...
builder.Services.AddCors(options =>
{
    ...
    options.AddPolicy(
        name: AllowLocalhostOriginsPolicy,
        policy =>
        {
            policy
    ...
    options.AddPolicy(
        name: AllowProductionOriginPolicy,
        policy =>
        {
            policy
                .WithOrigins("https://bazaar-admin-web.netlify.app") // YOUR specific frontend URL
                .AllowAnyHeader() // More permissive for dev
                .AllowAnyMethod()
                .AllowCredentials();
        }
...
if (!builder.Environment.IsEnvironment("Testing"))
{
    ...
    builder.Services.AddDbContext<NotificationsDbContext>(options =>
        options.UseNpgsql(builder.Configuration.GetConnectionString("NotificationsConnection"))
    );
    builder.Services.AddDbContext<InventoryDbContext>(options =>
        options.UseNpgsql(builder.Configuration.GetConnectionString("InventoryConnection"))
    );
    builder.Services.AddDbContext<ReviewDbContext>(options =>
        options.UseNpgsql(builder.Configuration.GetConnectionString("ReviewConnection"))
    );
    builder.Services.AddDbContext<AdDbContext>(options =>
        options.UseNpgsql(builder.Configuration.GetConnectionString("AdvertismentConnection"))
    );
    builder.Services.AddDbContext<ConversationDbContext>(options =>
        options.UseNpgsql(builder.Configuration.GetConnectionString("ConversationConnection"))
    );
    builder.Services.AddDbContext<TicketingDbContext>(options =>
        options.UseNpgsql(builder.Configuration.GetConnectionString("TicketingConnection"))
    );
...
builder
    .Services.AddIdentity<User, IdentityRole>() // Replace User if needed
