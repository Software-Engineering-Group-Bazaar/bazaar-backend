...
namespace SharedKernel
{
    public static class UserDataSeeder // Renamed for clarity
    {
        ...
        public static async Task SeedDevelopmentUsersAsync(this IHost host)
        {
            using (var scope = host.Services.CreateScope())
            {
                ...
                try
                {
                    ...
                    // --- Seed CORE Users with IsActive = true ---
                    ...
                    // --- Seed Additional Random Users (using default IsActive value) ---
                    ...
                    {
                        string email;
                        List<string> roles = new List<string> { basicUserRole };
                        ...
                        // Uses the default isActive value (true) from the helper
        ...
        private static async Task CreateUserWithRolesAsync(
            ...
            bool isActive = true) // <-- Added isActive parameter with default true
        {
            if (await userManager.FindByEmailAsync(email) == null)
            {
                logger.LogInformation("Creating user: {Email} with IsActive={IsActive}", email, isActive);
                var user = new User
                {
                    UserName = email,
                    Email = email,
                    EmailConfirmed = true,
                    IsApproved = true, // Keep IsApproved = true for all seeded dev users
                    IsActive = isActive // <-- Assign the isActive parameter
                };
                ...
                if (result.Succeeded)
                {
                    logger.LogInformation("User {Email} created successfully. Setting randomized CreatedAt.", email);

                    // Update CreatedAt timestamp
                    var createdUser = await userManager.FindByEmailAsync(email);
                    if (createdUser != null)
                    {
                        createdUser.CreatedAt = DateTime.UtcNow.AddDays(-Random.Shared.Next(1, 730)).AddHours(-Random.Shared.Next(0, 24));
                    ...
                    else { logger.LogWarning("Could not retrieve newly created user {Email} to update CreatedAt.", email); }
                    ...
                    {
                        logger.LogWarning("Failed to assign roles [{Roles}] to user {Email}. Errors: {Errors}", string.Join(", ", roles), email, string.Join(", ", rolesResult.Errors.Select(e => e.Description)));
                    }
                }
                else
                {
